...
NaruSecD3Js.createBarGraph = function(opt, data){
	...
	function setData(data){
		...
		if(objD3.y0Axis){
			objD3.y0Axis.tickSize(-(objD3.svgWidth - objD3.legendWidth));
			...
			if(animation && Object.keys(animation).length > 0){
				...
				objBars
					...
					.attr("height", function(d){
						//console.log( objD3.getSvgHeight() - objD3.getY0Scale(d[objD3.getY0Value()]));
...
//	}
	...
	var labelMaxLength = d3.max(objD3.data, function(d, i){
...
function createForce(objD3, objDraw){
	...
	var force = d3.layout.force()
				.size([objD3.getSvgWidth(), objD3.getSvgHeight()])
				.charge(-100)
...
//				.attr("height", "100%");
		...
		createGraph: function (chartId, svgMargin, className) {
			...
			//graphLayer.append("<div class='naru-chart-wrapper'></div>");
			...
			var objTemp = objSvg.append("g").attr("class", className).attr("transform", "translate(" + (this.svgMargin.left + this.axisX0 ) + "," + this.svgMargin.top + ")");
			return objTemp;
		},
		...
		setX0OutputRange: function () {
			if (this.x0Scale) {
				if (this.x0OutputType == "range") this.x0Scale.range([0, this.svgWidth - this.legendWidth]);
				else if (this.x0OutputType == "rangeRoundBands") this.x0Scale.rangeRoundBands([0, this.svgWidth - this.legendWidth], 0.1);
				else if (this.x0OutputType == "rangeBands") this.x0Scale.rangeBands([0, this.svgWidth - this.legendWidth]);
			}
		},

		setY0OutputRange: function () {
			if (this.y0Scale) {
				if (this.yOutputType == "range") this.y0Scale.range([this.svgHeight, 0]);
			}
		},

		setY1OutputRange: function () {
			if (this.y1Scale) {
				if (this.y1OutputType == "range") this.y1Scale.range([this.svgHeight, 0]);
			}
		},
		...
		resize: function () {
			...
			if (this.x0Axis) {
				this.x0Axis.tickSize(-this.svgHeight);
			...
			if (this.y1Axis) {
				this.y1Axis.tickSize(-(this.svgWidth - this.legendWidth));
